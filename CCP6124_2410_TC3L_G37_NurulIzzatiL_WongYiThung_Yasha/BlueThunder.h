...
public:
    ...
    BlueThunder(string id = "", int x = -1, int y = -1, int numOfLive = 3)
    {
        id_ = id;
        robotPositionX = x;
        robotPositionY = y;
    ...
    virtual void actionFire(ofstream &outputFile)
    {
        ...
        while (true)
        {
            ...
            if (fireX >= 0 && fireY >= 0 && fireX < battlefield()->getWidth() && fireY < battlefield()->getHeight()) {
                break;
            }
        }
        ...
        for (auto robot : battlefield()->getVecRobots())
        {
            if(robot->x() == fireX && robot->y() == fireY)
            {
                ...
                if(this->numOfKills_ == 3)
                {
                    ...
                    {
                        newId = "MB0" + to_string(intNewId);
                    }
                    else
                    {
                        newId = "MB" + to_string(intNewId);
                    }

                    robot->setId(newId);

                    cout << "\nBlueThunder has killed 3 robots" << endl;
                    outputFile << "\nBlueThunder has killed 3 robots" << endl;

                    cout << "BlueThunder has upgraded to MadBot." << endl;
                    outputFile << "BlueThunder has upgraded to MadBot." << endl;
...
int BlueThunder::robotAutoIncrement_ = 0;
